...
export default function MatchRegistryClient({
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Match Registry Tabs and View Toggle */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/10 pb-2">
        {/* Tabs */}
        ...
        {/* Segmented Grid vs Map Toggle */}
        <div className="inline-flex rounded-xl bg-slate-950/60 p-1 border border-white/5 backdrop-blur-md">
          <Link
            href={getQueryString({ view: 'grid' })}
            className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-all duration-200 flex items-center gap-1.5 cursor-pointer ${
              viewMode !== 'map'
                ? 'bg-gradient-to-r from-indigo-500/20 to-emerald-500/20 border border-indigo-500/30 text-indigo-300'
      ...
      {viewMode === 'map' ? (
        activeMatches.length > 0 ? (
          ...
          <div className="backdrop-blur-xl bg-white/5 border border-white/10 p-16 rounded-2xl flex flex-col justify-center items-center text-center space-y-6 max-w-xl mx-auto shadow-2xl animate-fadeIn">
            ...
            <div className="space-y-2">
              <h3 className="text-2xl font-bold text-white">No Matches Located</h3>
              <p className="text-slate-400 text-sm leading-relaxed">
                There are no matches fitting your search criteria to display on the map. Try expanding your search radius or modifying your location.
              </p>
            </div>
          </div>
        )
